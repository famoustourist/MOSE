...
def compute_v(
    ...
    #print(cur_input)
    #*************************new added  orome**********************
    #target=v, cur_output = W * k*, left_vector=KK^T
    
    
    A = weights[f"{hparams.rewrite_module_tmp.format(layer)}.weight"] @ KKT
    
    a_star = weights[f"{hparams.rewrite_module_tmp.format(layer)}.weight"] @ k_star
    
    print("the original a norm", a_star.norm())
    
    times= target.norm()/a_star.norm()
    
    a_star= a_star/a_star.norm()
    a_star = a_star.unsqueeze(-1)
    # original weight W0
    weight_name = f"{hparams.rewrite_module_tmp.format(layer)}.weight"
    W0 = torch.load(f"/home/jyma/oft_edit/results/zsre_weight/ROME/llama2-7b/original.pt")[weight_name]
    
    B = W0 @ KKT
    
    print("the original b norm", target.norm())
    

    b_star = target/target.norm()
    b_star = b_star.unsqueeze(-1)
    
    print("a_star:", a_star,a_star.shape,"\n")
    
    print("b_star:", b_star, b_star.shape,"\n")
    
    w = (a_star-b_star)/(a_star-b_star).norm()
    print(w, w.shape)
    #print(w.t() @ w)
    print(w @ w.t())
    
    I= torch.eye(w.shape[0]).to(f"cuda:{hparams.device}")
    ...
    u,s,v=np.linalg.svd(M.detach().cpu().numpy(),full_matrices=1,compute_uv=1)
    
    print("u, v shape", u.shape, v.shape)
    
    #???????
    S_p = torch.from_numpy(np.dot(u, v)).to(f"cuda:{hparams.device}")
    
    S = P @ S_p + b_star @ b_star.t()
    #v = target - cur_output
    ...
    '''
    print("verify the delta:", (R @ a_star -b_star).norm(),"\n\n")
