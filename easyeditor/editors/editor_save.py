    ...
    def __init__(self,
        ...
        if type(self.model_name) is str:
            ...
            if 't5' in self.model_name.lower():
                ...
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name,trust_remote_code=True, torch_dtype=torch.float32, device_map=device_map)
    ...
    def edit(self,
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE':
                ...
                edited_model, weights_copy, k, v = self.apply_algo(
                    self.model,
                    self.tok,
                    [request],
                    self.hparams,
                ...
                '''
                #add_weights
                if i==0 or i==4 or i==9 or i==14 or i==19 or i==24 or i==29 or i==39 or i==49 or i==59 or i==69 or i==79 or i==89 or i==99 or i==149 or i==199 or i==299 or i==499 or i==399 or i==699 or i==899 or i==999:

                    if not os.path.exists("results/zsre_weight/{}/{}".format(self.hparams.alg_name, self.hparams.model_name.split("/")[-1], i)): #判断所在目录下是否有该文件名的文件夹
                        os.makedirs("results/zsre_weight/{}/{}".format(self.hparams.alg_name, self.hparams.model_name.split("/")[-1], i)) #创建多级目录用mkdirs，单击目录mkdir

                    matrix_file = np.save("results/zsre_weight/{}/{}/save_{}".format(self.hparams.alg_name, self.hparams.model_name.split("/")[-1], i), upd_matrixs)
                '''
        ...
        for i, request in enumerate(tqdm(requests)):
            ...
            all_metrics[i].update({
                    "after-post": compute_edit_quality(edited_model, self.model_name, self.hparams, self.tok, request, self.hparams.device, test_generation=test_generation),
                })

            if 'locality' in all_metrics[i]['after-post'].keys():
                for locality_key in request['locality'].keys():
                    assert len(all_metrics[i]['after-post']['locality'][f'{locality_key}_output']) == \
                           len(all_metrics[i]['pre']['locality'][f'{locality_key}_output'])
                    locality_result = []
                    for ans,label in zip(all_metrics[i]['after-post']['locality'][f'{locality_key}_output'],all_metrics[i]['pre']['locality'][f'{locality_key}_output']):
                        locality_result.append(np.mean(np.equal(ans, label)))
                    all_metrics[i]['after-post']['locality'][f'{locality_key}_acc'] = locality_result
                    all_metrics[i]['after-post']['locality'].pop(f'{locality_key}_output')
                all_metrics[i]['pre'].pop('locality')
        ...
        if summary_metrics and len(all_metrics)!=0:
            ...
            mean_metrics = dict()
            for eval in ["pre", "post",'after-post']:
        ...
        return all_metrics, edited_model, weights_copy, ks, vs
